...
<div class="ik-header">
    <div style="display:flex; align-items:center; gap:12px;">
        <div class="ik-header-icon-badge"
            style="background:linear-gradient(135deg, #fef9c3 0%, #fef08a 100%); color:#ca8a04; box-shadow:0 2px 8px rgba(202, 138, 4, 0.15);">
        ...
        <div>
            ...
            <p class="ik-header-sub">Pusat peninjauan 2 pintu untuk persetujuan ide dari Creative Team dan hasil desain
                visual dari Content Creator.</p>
        </div>
    </div>
</div>
    ...
    <div class="ik-stat-card">
        <div class="ik-stat-icon amber">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
                <path
                    d="M9 18h6m-4 4h2m-6-8c0-2.8 2.2-5 5-5s5 2.2 5 5c0 1.6-.8 3-2 3.8v1.2H10v-1.2C8.8 16 8 14.6 8 13z" />
            </svg>
        </div>
        <div>
    ...
    <div class="cp-toolbar">
        <div class="cp-toggle-wrap">
            <a href="?status=all" class="cp-tog <?= ($filterStatus === 'all') ? 'active' : '' ?>">
                <svg class="ik-tog-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="2">
            ...
            <a href="?status=overdue" class="cp-tog <?= ($filterStatus === 'overdue') ? 'active' : '' ?>" style="<?= ($filterStatus === 'overdue') ? 'background:#ef4444;color:#fff;' : 'color:#dc2626;' ?>">
                <svg class="ik-tog-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <circle cx="12" cy="12" r="10" />
                    <line x1="12" y1="8" x2="12" y2="12" />
                    <line x1="12" y1="16" x2="12.01" y2="16" />
                </svg>
        ...
        <div class="cp-filters" style="display:flex; align-items:center; gap:8px;">
            ...
            <div style="position:relative;">
                ...
                <input type="text" id="searchQuery" class="cp-inp"
                    style="width:200px; padding:7px 12px 7px 34px; font-size:12.5px; border-radius:10px; border:1.5px solid #cbd5e1; outline:none;"
                    placeholder="Cari data review..." oninput="renderIdeList()">
            </div>
        </div>
    </div>
        ...
        <table class="cp-table">
            <thead>
                <tr>
                    ...
                    <th style="width:16%;">Status Saat Ini</th>
                    <th style="width:14%;">Platform</th>
                    <th style="width:18%; white-space:nowrap;">Jadwal & Waktu</th>
                    <th style="width:12%;">Pembuat</th>
                    <th style="text-align:right; width:14%;">Aksi</th>
                </tr>
            </thead>
                ...
                <?php else: ?>
                    <?php foreach ($konten as $i => $k): ?>
                        <tr class="cp-tr-item" onclick="bukaDetail(<?= $k['id'] ?>)" style="cursor:pointer">
                            ...
                            <td>
                                <div
                                    style="font-weight:700; font-size:13.5px; color:#0f172a; line-height:1.4; margin-bottom:2px;">
                                    <?= esc($k['judul_konten']) ?></div>
                                <?php if (!empty($k['deskripsi'])): ?>
                                    <div
                                        style="font-size:12px; color:#64748b; line-height:1.4; max-width:320px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                        <?= esc($k['deskripsi']) ?>
                                    </div>
                                <?php endif; ?>
                            </td>
                            <td>
                                ...
                                ?>
                                <span class="cp-badge <?= $sCls ?>"
                                    style="padding:4px 12px; font-size:12px; display:inline-block; font-weight:600; border-radius:20px; white-space:nowrap;">
                            ...
                            <td style="font-size:13px; color:#475569; font-weight:500;">
                                <span
                                    style="display:inline-flex; align-items:center; gap:5px; background:#f1f5f9; padding:3px 10px; border-radius:12px; font-size:12px; color:#334155; font-weight:600;">
                                    <?= esc($k['nama_pembuat'] ?: '—') ?>
                                </span>
                            </td>
                            <td style="text-align:right" onclick="event.stopPropagation()">
                                <button class="cpb cpb-pri" onclick="bukaDetail(<?= $k['id'] ?>)"
                                    style="padding:6px 14px; font-size:12px; font-weight:600; border-radius:8px; white-space:nowrap; background:#2563eb; color:#fff;">
                                    Review & Keputusan
                                </button>
                            </td>
                        </tr>
...
<div class="cp-back" id="backDet">
    <div class="cp-modal modal-lg">
        ...
        <div class="cp-tab-panel active" id="panelInfo">
            ...
            <!-- Summary Banner -->
            <div
                style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; padding:12px 16px; margin-bottom:18px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                <div style="display:flex; align-items:center; gap:10px;">
                    <div class="cp-det-label" style="font-size:12px; font-weight:700; color:#64748b; margin:0;">Status
                        Saat Ini:</div>
            ...
            <div
                style="border:1.5px solid #e2e8f0; border-radius:14px; padding:18px; background:#ffffff; margin-bottom:18px; box-shadow:0 2px 8px rgba(0,0,0,0.02);">
                <div
                    style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; border-bottom:1px solid #f1f5f9; padding-bottom:10px;">
                    <div
                        style="font-size:14px; font-weight:800; color:#0f172a; display:flex; align-items:center; gap:8px;">
                    ...
                    <span
                        style="font-size:11.5px; font-weight:600; color:#2563eb; background:#eff6ff; padding:3px 10px; border-radius:12px;">Akses
                        Penuh Edit</span>
                </div>
                ...
                <div class="cp-field" style="margin-bottom:14px;">
                    <label
                        style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Judul
                        Konten <span style="color:#dc2626">*</span></label>
                    <input type="text" id="fMgrJudul" class="cp-inp" placeholder="Judul konten..."
                        style="padding:10px 14px; font-size:13.5px; font-weight:600;">
                </div>
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:12px; margin-bottom:14px;">
                    <div class="cp-field">
                        <label
                            style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Jadwal
                            & Jam Publish</label>
                        <input type="datetime-local" id="fMgrTanggal" class="cp-inp"
                            style="padding:9px 12px; font-size:12.5px;">
                    </div>
                    <div class="cp-field">
                        <label
                            style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Jenis
                            Konten</label>
                    ...
                    <div class="cp-field">
                        <label
                            style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Content
                            Pillar</label>
                ...
                <div class="cp-field" style="margin-bottom:14px;">
                    <label
                        style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Platform
                        Target</label>
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px; margin-bottom:14px;">
                    <div class="cp-field">
                        <label
                            style="font-size:12px; font-weight:700; color:#374151; margin-bottom:6px; display:block;">Tugaskan
                            Designer (Content Creator)</label>
                        <select id="fMgrDesigner" class="cp-inp" style="padding:9px 12px; font-size:12.5px;">
                            ...
                            <?php foreach ($designers as $u): ?>
                                <option value="<?= $u['id'] ?>"><?= esc($u['nama']) ?> (<?= esc($u['nama_role'] ?? 'Designer') ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="cp-field">
                        ...
                        <select id="fMgrUploader" class="cp-inp" style="padding:9px 12px; font-size:12.5px;">
                            ...
                            <?php foreach ($uploaders as $u): ?>
                                <option value="<?= $u['id'] ?>"><?= esc($u['nama']) ?> (<?= esc($u['nama_role'] ?? 'Uploader') ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="cp-field" style="margin-bottom:14px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                        <label style="font-size:12px; font-weight:700; color:#374151; margin:0;">Deskripsi / Brief
                            Konsep Visual</label>
                        <button type="button" class="cpb cpb-sec" id="btnMgrAiBrief"
                            style="padding:4px 10px; font-size:11.5px; font-weight:600; display:inline-flex; align-items:center; gap:4px; border-radius:6px; background:#e0f2fe; border:1px solid #bae6fd; color:#0284c7;"
                            onclick="generateAiBriefManager()">
                    ...
                    <textarea id="fMgrDesc" class="cp-inp"
                        placeholder="Tulis brief ide, poin penting, arahan revisi atau konsep..." rows="3"
                        style="padding:10px 14px; min-height:75px; font-size:13px; line-height:1.5;"></textarea>
                </div>
                ...
                <div class="cp-field" style="margin-bottom:14px;">
                    <div
                        ...
                        <button type="button" class="cpb cpb-sec" id="btnAiCaption"
                            style="padding:4px 10px; font-size:11.5px; display:inline-flex; align-items:center; gap:4px; background:#f0f9ff; border:1px solid #bae6fd; color:#0284c7; font-weight:600; border-radius:6px;"
                            onclick="generateAiCaption()">
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2">
                                <path
                                    d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" />
                            </svg> Tulis Caption AI
                        </button>
                    </div>
                    <textarea id="inCaptionText" class="cp-inp" rows="3"
                        placeholder="Tulis caption manual atau gunakan bantuan AI..."
                        style="width:100%; font-size:13px; padding:10px 12px; border-radius:8px; line-height:1.5; resize:vertical; min-height:75px;"></textarea>
                </div>

                <!-- Tombol Simpan Perubahan Data -->
                <div style="display:flex; justify-content:flex-end; padding-top:6px;">
                    <button type="button" class="cpb cpb-pri" id="btnSimpanMgrEdit" onclick="simpanManagerEdit()"
                        style="padding:9px 20px; font-size:12.5px; font-weight:700; border-radius:8px; display:inline-flex; align-items:center; gap:6px; background:#2563eb; color:#fff;">
            ...
            <!-- Link Desain & Preview Materi Konten (Hanya tampil saat status bukan ide_diajukan) -->
            <div id="wrapMateriDesain" style="display:grid; grid-template-columns: 1fr 1fr; gap:12px; margin-bottom:18px;">
                <!-- Link Desain Canva Box -->
                <div class="cp-design-box" id="designBox"
                    style="border:1px solid var(--cp-border); border-radius:12px; padding:14px; background:var(--cp-white);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <div
                            style="font-weight:700; font-size:12.5px; color:var(--cp-text); display:flex; align-items:center; gap:6px;">
                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#00c4cc"
                                stroke-width="2">
                                <path d="M12 19l7-7 3 3-7 7-3-3z" />
                                <path d="M18 13l-1.5-7.5L2 2l3.5 14.5L13 18l5-5z" />
                                <circle cx="11" cy="11" r="2" />
                        ...
                        <a id="btnBukaCanva" class="cpb cpb-sec" target="_blank" rel="noopener noreferrer"
                            style="padding:5px 10px; font-size:11.5px; text-decoration:none; display:inline-flex; align-items:center; gap:4px; background:#f0fdf4; border:1px solid #bbf7d0; color:#16a34a; font-weight:600; border-radius:6px;">
                            Buka Desain ↗
                        </a>
                    </div>
                    <div style="display:flex; gap:6px; align-items:center;">
                        <input type="url" id="inDesignUrl" class="cp-inp" placeholder="Link Canva/Figma..."
                            style="flex:1; font-size:12px; padding:7px 10px; border-radius:6px;">
                        <button type="button" class="cpb cpb-pri" id="btnSimpanDesignUrl" onclick="simpanDesignUrl()"
                            style="padding:7px 12px; font-size:11.5px; font-weight:600; white-space:nowrap; border-radius:6px;">
                            Simpan
                        </button>
                    </div>
                </div>
                ...
                <div class="cp-upload-box" id="uploadImageBox"
                    style="border:1px solid var(--cp-border); border-radius:12px; padding:14px; background:var(--cp-white);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <div
                            style="font-weight:700; font-size:12.5px; color:var(--cp-text); display:flex; align-items:center; gap:6px;">
                        ...
                        <a id="btnBukaGambar" class="cpb cpb-sec" target="_blank" rel="noopener noreferrer"
                            style="padding:5px 10px; font-size:11.5px; text-decoration:none; display:inline-flex; align-items:center; gap:4px; background:#f0fdf4; border:1px solid #bbf7d0; color:#16a34a; font-weight:600; border-radius:6px;">
                            Preview ↗
                        </a>
                    </div>
                    <div style="display:flex; gap:6px; align-items:center;">
                        <input type="text" id="inImageUrl" class="cp-inp" placeholder="Link Google Drive..."
                            style="flex:1; font-size:12px; padding:7px 10px; border-radius:6px;">
                        <button type="button" class="cpb cpb-pri" id="btnSimpanImageUrl" onclick="simpanImageUrl()"
                            style="padding:7px 12px; font-size:11.5px; font-weight:600; white-space:nowrap; border-radius:6px;">
                            Simpan
                        </button>
                    </div>
            ...
            <div class="cp-transition-box" id="transitionBox"
                style="display:none; margin-top:0; border:1.5px solid #cbd5e1; border-radius:14px; padding:18px; background:#f8fafc;">
                <div class="cp-transition-label"
                    style="display:flex; align-items:center; gap:6px; font-size:13.5px; font-weight:800; color:#0f172a; margin-bottom:12px;">
                ...
                <div class="cp-trans-field" style="margin-bottom:14px;">
                    <label
                        style="margin-bottom:8px; display:block; font-size:12px; font-weight:700; color:#374151;">Pilih
                        Keputusan Status:</label>
                ...
                <div class="cp-catatan-wrap" style="margin-bottom:12px;">
                    <label
                        style="margin-bottom:6px; display:block; font-size:12px; font-weight:700; color:#374151;">Catatan
                        Instruksi Revisi / Feedback Manager:</label>
                    <textarea class="cp-inp" id="txCatatan"
                        placeholder="Tulis instruksi perbaikan untuk Tim Kreatif / Designer (wajib jika memilih status Revisi atau Ditolak)..."
                        rows="3" style="padding:10px 14px; font-size:13px; min-height:80px;"></textarea>
                ...
                <div class="cp-catatan-wrap" id="wrapLinkPost" style="display:none; margin-bottom:12px;">
                    <label
                        style="margin-bottom:6px; display:block; font-size:12px; font-weight:700; color:#374151;">Link
                        Postingan Publikasi:</label>
                    <input type="url" class="cp-inp" id="inLinkPost"
                        placeholder="https://instagram.com/p/... (wajib untuk published)">
                ...
                <div style="margin-top:14px; display:flex; justify-content:flex-end;">
                    <button class="cpb cpb-pri" id="btnEksekusi" onclick="eksekusiTransisi()"
                        style="width:100%; padding:11px; font-size:13.5px; font-weight:800; border-radius:10px; background:#059669; color:#fff;">
...
<script>
    ...
    function populateManagerForm(k) {
        ...
        // Sembunyikan form hasil desain jika status masih ide_diajukan (Creative team tidak desain)
        const isIdeDiajukan = (k.status === 'ide_diajukan');
        const wrapMateri = document.getElementById('wrapMateriDesain');
        if (wrapMateri) wrapMateri.style.display = isIdeDiajukan ? 'none' : 'grid';
    ...
    async function simpanManagerEdit() {
        ...
        fd.append('design_url', document.getElementById('inDesignUrl')?.value || '');
        fd.append('image_url', document.getElementById('inImageUrl')?.value || '');
        ...
        if (res.status === 'sukses') {
            ...
            if (idx !== -1) {
                ...
                window.ALL_KONTEN[idx].design_url = document.getElementById('inDesignUrl')?.value || '';
                window.ALL_KONTEN[idx].image_url = document.getElementById('inImageUrl')?.value || '';
            }
        } else {
